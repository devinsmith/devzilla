                      ...
                      nsWidgetInitData *aInitData)
{
  ...
  if (gFirstTopLevelWindow == 0) {
    ...
    Widget shell = ::XtVaCreatePopupShell(" ",
        ...
        (Widget) aAppShell->GetNativeData(NS_NATIVE_SHELL), NULL);
    XtVaSetValues(shell, 
        XmNwidth, aRect.width, XmNheight, aRect.height, NULL);
    mainWindow = ::XtVaCreateManagedWidget("mainWindow",
				 xmMainWindowWidgetClass,
				 shell, 
         NULL);
                      ...
                      nsWidgetInitData *aInitData)
{
    ...
    if (aContext) {
      ...
      NS_DEFINE_IID(kDeviceContextCID, NS_DEVICE_CONTEXT_CID);
      NS_DEFINE_IID(kDeviceContextIID, NS_IDEVICE_CONTEXT_IID);
...
NS_METHOD nsWindow::Resize(PRUint32 aWidth, PRUint32 aHeight, PRBool aRepaint)
{
  ...
  XtVaSetValues(mWidget, XmNx, mBounds.x, XmNy, mBounds.y, XmNwidth, aWidth, XmNheight, aHeight, NULL);
  return NS_OK;
}
...
NS_METHOD nsWindow::SetBackgroundColor(const nscolor &aColor)
{
  ...
  PRUint32 pixel;
  mContext->ConvertPixel(aColor, pixel);
  XtVaSetValues(mWidget, XtNbackground, pixel, NULL);
  return NS_OK;
}
#if 0
...
PRBool nsWindow::OnPaint(nsPaintEvent &event)
{
  ...
  if (mEventCallback) {
    ...
    NS_DEFINE_IID(kRenderingContextCID, NS_RENDERING_CONTEXT_CID);
    NS_DEFINE_IID(kRenderingContextIID, NS_IRENDERING_CONTEXT_IID);
...
extern "C" void nsWindow_ResizeWidget(Widget w)
{
  ...
  XtVaGetValues(w, XmNuserData, &win, XmNwidth, &width, XmNheight, &height, NULL);

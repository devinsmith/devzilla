...
LRESULT CALLBACK nsWindow::WindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    nsWindow *someWindow = (nsWindow*)::GetWindowLongPtr(hWnd, GWLP_USERDATA);
    ...
    if (msg == WM_NOTIFY) {
      ...
      if (pnmh->code == TCN_SELCHANGE) {             
        someWindow = (nsWindow*)::GetWindowLongPtr(pnmh->hwndFrom, GWLP_USERDATA); 
      }
    }
                      ...
                      nsNativeWidget aNativeParent)
{
    ...
    if (! toolkit->IsGuiThread()) {
        LPARAM args[5];
        args[0] = (LPARAM)aParent;
        args[1] = (LPARAM)&aRect;
        args[2] = (LPARAM)aHandleEventFunction;
        args[3] = (LPARAM)aContext;
        args[4] = (LPARAM)aToolkit;
        args[5] = (LPARAM)aInitData;
...
nsIWidget* nsWindow::GetParent(void)
{
    ...
    if (mWnd) {
        ...
        if (parent) {
            widget = (nsWindow *)::GetWindowLongPtr(parent, GWLP_USERDATA);
            if (widget) {
...
NS_METHOD nsWindow::SetBackgroundColor(const nscolor &aColor)
{
    ...
    mBrush = ::CreateSolidBrush(NSRGB_2_COLOREF(mBackground));
    if (mWnd != NULL) {
      SetClassLongPtr(mWnd, GCLP_HBRBACKGROUND, (LPARAM)mBrush);
    }
    return NS_OK;
}
    ...
    *aRetValue = 0;
    ...
    switch (msg) {
        ...
        case WM_VSCROLL:
	          ...
	          if (lParam) {
                nsWindow* scrollbar = (nsWindow*)::GetWindowLongPtr((HWND)lParam, GWLP_USERDATA);
        ...
        case WM_CTLCOLORSTATIC:
	          if (lParam) {
              nsWindow* control = (nsWindow*)::GetWindowLongPtr((HWND)lParam, GWLP_USERDATA);
		          if (control) {
...
void nsWindow::SubclassWindow(BOOL bState)
{
  if (NULL != mWnd) {
    ...
    if (bState) {
        ...
        mPrevWndProc = (WNDPROC)::SetWindowLongPtr(mWnd, GWLP_WNDPROC, 
                                                 (LPARAM)nsWindow::WindowProc);
        NS_ASSERTION(mPrevWndProc, "Null standard window procedure");
        // connect the this pointer to the nsWindow handle
        ::SetWindowLongPtr(mWnd, GWLP_USERDATA, (LPARAM)this);
    } 
    else {
        ::SetWindowLongPtr(mWnd, GWLP_WNDPROC, (LPARAM)mPrevWndProc);
        ::SetWindowLongPtr(mWnd, GWLP_USERDATA, (LPARAM)NULL);
        mPrevWndProc = NULL;
    }
  }
}

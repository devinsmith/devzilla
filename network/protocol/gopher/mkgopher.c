...
net_parse_menu (ActiveEntry * cur_entry)
{
    ...
    if (gopher_type && *ptr) 
	  {
        name = ptr;
        gopher_path = strchr(name, '\t');
        if (gopher_path) 
		  {
            *gopher_path++ = 0;
            host = strchr(gopher_path, '\t');
            if (host) 
			  {
                *host++ = 0;    
                port = strchr(host, '\t');
...
net_GopherLoad (ActiveEntry * cur_entry)
{
    ...
    switch(gopher_type) {
        case GTYPE_INDEX:
            /* Search is allowed */
            query = strchr(CE_URL_S->address, '?');  /* Look for search string */

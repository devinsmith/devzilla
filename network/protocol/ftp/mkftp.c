    ...
    if (cd->response_code == 2)
      {
        ...
        else if (strstr(cd->return_msg, "UNIX") != NULL)
          {
             cd->cc->server_type = FTP_UNIX_TYPE;
             cd->cc->use_list = TRUE;
          }
        else if (strstr(cd->return_msg, "Windows_NT") != NULL)
          {
        ...
        else if (strstr(cd->return_msg, "MAC-OS TCP/Connect II") != NULL)
          {
...
{
    FTPConData * cd = (FTPConData *)ce->con_data;
    char *cp = strchr(cd->return_msg+5,'"');
        ...
        if ((cp=strrchr(cd->path, '/'))!=NULL) 
          {
			...
			*cp = '/';  /* set it back so it is left unmodified */
			...
			/* turn slashes into '.' */
        	while ((cp1=strchr(cd->output_buffer, '/')) != NULL)
            	*cp1 = '.';
          }
        else
          {
...
net_start_ftp_read_dir(ActiveEntry * ce)
{
    ...
    if(cd->cwd_message_buffer)
    {
		 ...
		 *                        101: message line 2, etc...
	 	 */
		char *end_line = strchr(cd->cwd_message_buffer, '\n');
		if(end_line)
			*end_line = '\0';
...
net_parse_vms_dir_entry (char *line, NET_FileEntryInfo *entry_info)
{
        ...
        if (!PL_strlen(line) || (cp=strchr(line, ';')) == NULL) 
		  {
        ...
        switch (server_type)
        {
            ...
            case FTP_MACHTEN_TYPE:
                ...
                if(!PL_strncmp(entry, "total ", 6)  
						|| (strstr(entry, "Permission denied") != NULL)
...
net_FTPLoad (ActiveEntry * ce)
{
     ...
     */
    if((semi_colon = strrchr(cd->path, ';')) != NULL)
      {

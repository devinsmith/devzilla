...
PR_IMPLEMENT(PRStatus) PR_GetSystemInfo(PRSysInfo cmd, char *buf, PRUint32 buflen)
{
    ...
    switch(cmd)
    {
      ...
      case PR_SI_SYSNAME:
        ...
        (void)snprintf(buf, buflen, _PR_SI_SYSNAME);
        break;
      ...
      case PR_SI_RELEASE:
...
#if defined(XP_UNIX)
        {
            struct utsname info;
            uname(&info);
            (void)snprintf(buf, buflen, info.release);
        }
#endif
#if defined(XP_OS2)
        {
            ...
            if (os2ver[0] < 30)
              (void)snprintf(buf, buflen, "%s%lu",
                                "2.", os2ver[0]);
            else
              (void)snprintf(buf, buflen, "%lu%s%lu",
      ...
      case PR_SI_ARCHITECTURE:
        ...
        (void)snprintf(buf, buflen, _PR_SI_ARCHITECTURE);
        break;
    }
    return PR_SUCCESS;
}

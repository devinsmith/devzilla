...
nsCmdLineService::Initialize(int argc, char ** argv)
{
  ...
  mArgList.AppendElement((void *)strdup("-progname"));
  mArgValueList.AppendElement((void *)strdup(argv[0]));
  mArgCount++;
  i++;
  ...
  for(i=1; i<argc; i++) {
    ...
    if (argv[i][0] == '-') {
     /* An option that starts with -. May or many not
	    ...
	    */
	   mArgList.AppendElement((void *)strdup(argv[i]));
     ...
     //Look ahead if this option has a value like -w 60
	   ...
	   if (i == argc) {
	      ...
	      */
	     mArgValueList.AppendElement((void *)strdup("1"));
	     mArgCount++;
	     break;
	   }
     if (argv[i][0] == '-') {
         ...
         * load if this is the last argument and has a ':/' in it.
	      ...
	      if ((i == (argc-1)) && (strstr(argv[i], ":/"))) {
           ...
           mArgList.AppendElement((void *)strdup("-url"));
		       mArgValueList.AppendElement((void *) strdup(argv[i]));
	 	       mArgCount++;
           continue;
        }
	      else {
...
nsCmdLineService::GetCmdLineValue(char * arg, char ** aResult)
{
   ...
   {
     if (!strcmp(arg, (char *) mArgList.ElementAt(i))) {

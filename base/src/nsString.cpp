...
nsString& nsString::Append(PRInt32 aInteger,PRInt32 aRadix) {
  ...
  char buf[40];
  snprintf(buf, sizeof(buf), fmt, aInteger);
  Append(buf);
  return *this;
}
...
nsString& nsString::Append(float aFloat){
  char buf[40];
  snprintf(buf, sizeof(buf), "%g", aFloat);
  Append(buf);
  return *this;
}
...
PRInt32 nsString::FindCharInSet(const char* anAsciiSet,PRInt32 anOffset) const{
  ...
  if(anAsciiSet && (strlen(anAsciiSet))) {
    for(PRInt32 i=anOffset;i<mLength;i++){
      const char* pos=strchr(anAsciiSet,char(mStr[i]));
      if(pos)
        return i;
    }
  }
 ...
 * @return
 */
void nsString::DebugDump(std::ostream& aStream) const {
 ...
 * @return
 */
std::ostream& operator<<(std::ostream& os,nsAutoString& aString){
	const PRUnichar* uc=aString.GetUnicode();
	int len=aString.Length();
	for(int i=0;i<len;i++)
		os<<(char)uc[i];
